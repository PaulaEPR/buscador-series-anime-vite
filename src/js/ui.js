...
import { listenerCards, listenDeleteBtns } from './events.js';
import { eraseSection } from './helpers.js';
...
function setBkg(element, imgUrl) {
  const imgDefault =
    'https://placehold.co/225x350/fff/666/?text=No+Picture';
  if (
...
function createResults() {
  ...
  dom.mainRes.appendChild(resultSection);
  listenerCards();
}

// Paint favorites
function createFavs() {
  eraseSection('.js-favorites', dom.mainFav);
  const favSection = createTagJS('section', 'fav', 'js-favorites');
  for (const item of state.favorite) {
    const article = createListArticle(item.title, item.images.jpg.small_image_url, item.mal_id);
    favSection.appendChild(article);
  }
  dom.mainFav.appendChild(favSection);
  listenDeleteBtns();
}
